    ...
    @else
        <div class="flex flex-wrap gap-[1px] mb-6 text-sm bg-[var(--surface-1)] border border-[var(--border)] rounded-lg p-1 w-fit max-w-full">
            @foreach ($cards as $card)
                <a href="{{ route('cards.index', ['card' => $card->id]) }}"
                    class="px-4 py-2 rounded-md transition-colors whitespace-nowrap {{ $selected && $selected->id === $card->id ? 'bg-[var(--surface-2)] text-[var(--text)] font-medium' : 'text-[var(--text-dim)]' }}">
                    {{ $card->account_name }}
                </a>
            @endforeach
        </div>
        ...
        @if ($selected)
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-6">
                ...
                <form method="POST" action="{{ route('cards.destroy', $selected->id) }}"
                    onsubmit="return confirm('Tem certeza que deseja excluir o cartão \'{{ $selected->account_name }}\'? Todos os dados relacionados a ele (compras, parcelas e projeções) serão apagados permanentemente. Essa ação não pode ser desfeita.');">
                    @csrf
                    @method('DELETE')
            ...
            <div class="overflow-x-auto mb-6 -mx-4 px-4 sm:mx-0 sm:px-0">
                <div class="inline-flex gap-[1px] text-sm bg-[var(--surface-1)] border border-[var(--border)] rounded-lg p-1">
                    @foreach (['compras' => 'Compras', 'projecao' => 'Projeção de faturas', 'parcelamentos' => 'Parcelamentos'] as $key => $label)
                        <a href="{{ route('cards.index', ['card' => $selected->id, 'tab' => $key]) }}"
